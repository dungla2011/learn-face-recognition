...
BATCH_SIZE = 1  # Có thể tăng lên nếu GPU mạnh
frames_batch = []
batch_ids = []
...
while True:
    ...
    # Sử dụng GPU nếu có
    results = model(frame, verbose=False)[0]  # Turn off verbose for better performance
    yolo_end = time.time()
    ...
    # Optimize image saving with batch processing for cropped images
    crop_batch = []
    path_batch = []
    ...
    cv2.putText(frame, f"Save: {(save_end - save_start)*1000:.1f}ms", (10, 210), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    cv2.putText(frame, f"FPS: {fps:.1f}", (10, 240), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
    cv2.putText(frame, f"Using: {device}", (10, 270), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
